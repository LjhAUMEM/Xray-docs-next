...
FinalMask выполняет последний уровень маскировки трафика после того, как ядро завершит обработку шифрования транспортного уровня, включая TLS/REALITY.
...
Используется с транспортными уровнями raw | httpupgarde | websocket | grpc | xhttp.
...
`clients[n][m].delay`: В миллисекундах. Если 0, то отправляется вместе с предыдущим пакетом (sticky packet).

`clients[n][m].rand`: Добавление случайных байтов указанной длины, конфликтует с `packet`.

`clients[n][m].randRange`: Диапазон случайных байтов, по умолчанию 0-255.

`clients[n][m].type`: Тип `packet`: `array | str | hex | base64`, по умолчанию array.
...
Используется с транспортными уровнями raw udp | kcp | hysteria | xhttp h3.

`header-custom`: Всегда объединяется в заголовок пакета.

`header-dns`: DNS-маскировка из оригинального mKCP. Некоторые университетские сети разрешают DNS-запросы без авторизации; добавляет заголовок DNS к KCP.

`header-dtls`: DTLS-маскировка из оригинального mKCP. Маскирует под пакеты DTLS 1.2. Дополнительные настройки не требуются.

`header-srtp`: SRTP-маскировка из оригинального mKCP. Маскирует под пакеты SRTP, которые будут распознаваться как данные видеовызовов (например, FaceTime). Дополнительные настройки не требуются.

`header-utp`: uTP-маскировка из оригинального mKCP. Маскирует под пакеты uTP, которые будут распознаваться как данные загрузки BT. Дополнительные настройки не требуются.

`header-wechat`: Маскировка WeChat Video из оригинального mKCP. Маскирует под пакеты видеовызовов WeChat. Дополнительные настройки не требуются.

`header-wireguard`: WireGuard-маскировка из оригинального mKCP. Маскирует под пакеты WireGuard (не является настоящим протоколом WireGuard). Дополнительные настройки не требуются.

`mkcp-original`: Простая обфускация, которая когда-то применялась по умолчанию в mKCP. Возможно, вам потребуется настроить её для подключения к старым серверам mKCP. Дополнительные настройки не требуются.

`mkcp-aes128gcm`: Соответствует функции `seed` из оригинального mKCP. Использует AES-128-GCM для обфускации.

`noise`: Шум, отправляемый перед передачей данных.

`salamander`: Обфускация Salamander (из Hysteria2).

`sudoku`:

`xdns`: Использование DNS-запросов для передачи данных (аналогично DNSTT). Будет выполнять стандартные запросы DNS TXT для передачи полезной нагрузки.

Из-за технических ограничений предоставляемый MTU очень мал, что делает невозможным использование QUIC, рекомендуется использовать вместе с mKCP. Рекомендуемые значения MTU: 130 для клиента, 900 для сервера.

Поскольку выполняемые запросы являются стандартными, они могут быть перенаправлены через любой UDP DNS-сервер, хотя эффективность может быть крайне неудовлетворительной.

Для использования этой функции необходимо, чтобы сервер прослушивал порт 53, а протокол проксирования указывал на DNS-сервер (например, 8.8.8.8:53). Также у вас должен быть домен `domain`, NS-записи которого указывают на сервер.

Например, если у вас есть example.com, установите A-запись для a.example.com на IP-адрес, установите NS-запись для t.example.com на a.example.com, и в итоге используйте t.example.com. Домен для A-записи не должен быть поддоменом для NS-записи.

`xicmp`: Требует прав как минимум `CAP_NET_RAW` и должен находиться на самом внешнем слое, то есть быть первым в массиве. Не может использоваться вместе с `udpHop` и `dialerProxy`.
...
`client[n].rand`: Добавление случайных байтов указанной длины, конфликтует с `packet`.

`client[n].randRange`: Диапазон случайных байтов, по умолчанию 0-255.

`client[n].type`: Тип `packet`: `array | str | hex | base64`, по умолчанию array.
...
`noise[n].rand`: Добавление случайных или указанной длины случайных байтов, конфликтует с `packet`.

`noise[n].randRange`: Диапазон случайных байтов, по умолчанию 0-255.

`noise[n].type`: Тип `packet`: `array | str | hex | base64`, по умолчанию array.

`noise[n].packet`: Добавление фиксированных данных, конфликтует с `rand`.

`noise[n].delay`: В миллисекундах. После отправки шума задерживается на указанное время перед отправкой следующего.
...
`listenIp`: Прослушиваемый IP-адрес.

`id`: Если на одном IP-адресе находится несколько клиентов, рекомендуется, чтобы на сервере значение оставалось равным 0.
...
Включение логов bbr/brutal congestion control.
...
Формат удобен для пользователя и поддерживает различные общепринятые способы записи бит в секунду, включая `1000000`, `100kb`, `20 mb`, `100 mbps`, `1g`, `1 tbps` и т.д. Регистр не имеет значения, между значением и единицей измерения может быть пробел или его отсутствие. Если единица измерения не указана, по умолчанию используется bps (бит в секунду). Значение не может быть меньше 65535 bps.

Поведение согласования совпадает с оригинальной Hysteria:

Значение на сервере ограничивает максимальную скорость режима Brutal, которую может выбрать клиент. Значение 0 означает отсутствие ограничений для клиента.

Если на клиенте установлено значение 0, это означает использование режима BBR. Если значение отлично от 0, используется режим Brutal, который будет ограничен сервером.

Обратите внимание на принцип относительности: загрузка сервера — это скачивание клиента, а скачивание сервера — это загрузка клиента.

> `udpHop`: {"ports": string, "interval": number}

Настройка прыжков портов (port hopping) UDP.

`ports` — это диапазон портов для прыжков. Может быть строкой числового типа, например, `"1234"`, или диапазоном чисел, например, `"1145-1919"`, что означает 775 портов от 1145 до 1919. Можно использовать запятые для сегментации, например, `11,13,15-17` означает 5 портов: 11, 13, и порты с 15 по 17.

`interval` — интервал прыжков портов в секундах. Минимальное значение — 5, по умолчанию — 30 секунд.
...
Эти четыре параметра являются конкретными параметрами окон QUIC. **Не рекомендуется изменять эти значения, если вы не понимаете полностью, что делаете**. Если вы всё же хотите их изменить, рекомендуется поддерживать соотношение окна приема потока к окну приема соединения на уровне 2:5.

> `maxIdleTimeout`: number

Максимальное время простоя (в секундах). Время, через которое сервер закроет соединение, если не получит никаких данных от клиента. Диапазон: от 4 до 120 секунд, по умолчанию — 30 секунд.

> `keepAlivePeriod`: number

Интервал QUIC KeepAlive (в секундах). Диапазон: от 2 до 60 секунд. По умолчанию отключено.

> `disablePathMTUDiscovery`: bool

Отключить ли обнаружение MTU пути (Path MTU Discovery).

В других реализациях для ОС, отличных от Linux, Windows и Darwin, это принудительно отключено. В Xray это не принудительно. Если вы используете ОС, отличную от Linux, Windows или Darwin, вам может потребоваться отключить это вручную.

> `maxIncomingStreams`: number

Параметр сервера. Если установлен, не может быть меньше 8.
